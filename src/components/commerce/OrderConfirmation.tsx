...
import { PICKUP_READY_ESTIMATE, type CheckoutOrder } from "@/lib/checkout";
...
export function OrderConfirmation({ order, className }: OrderConfirmationProps) {
  ...
  return (
    <div className={["flex flex-col items-center text-center", className ?? ""].filter(Boolean).join(" ")}>
      ...
      <h1 className="mt-4 text-2xl font-extrabold tracking-tight text-primary">Order confirmed</h1>
      <p className="mt-1.5 text-sm text-secondary">
        Order <span className="font-mono font-semibold text-primary">#{order.orderNumber}</span> ·{" "}
        {placedDate.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}
      </p>
      <p className="mt-1 text-sm text-secondary">We&apos;ve sent a confirmation to {order.email}.</p>
        ...
        {order.deliveryMethod === "pickup" ? (
          <>
            ...
            <span className="text-[13.5px] text-secondary">
              <span className="block font-semibold text-primary">Same-day pickup — {PICKUP_READY_ESTIMATE}</span>
          ...
          <>
            ...
            <span className="text-[13.5px] text-secondary">
              <span className="block font-semibold text-primary">Local delivery — next business day</span>
              {order.address && (
                <>
